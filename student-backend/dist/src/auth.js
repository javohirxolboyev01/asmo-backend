...
const publicUser = { id: true, email: true, firstName: true, lastName: true, phone: true, avatar: true, role: true, status: true };
const id = () => crypto.randomUUID();
export const issueAccessToken = (user) => jwt.sign({ role: user.role }, secret, { subject: user.id, expiresIn: (process.env.JWT_EXPIRES_IN ?? '15m') });
...
export async function register(input) {
    ...
    const user = await prisma.$transaction(async (tx) => {
        ...
        if (role === 'TEACHER')
            await tx.teacher.create({ data: { id: id(), userId: created.id, fullName: `${created.firstName} ${created.lastName}` } });
        return created;
    });
